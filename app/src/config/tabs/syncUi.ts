...
export const refreshLANSyncConfigItemVisibility = (root: Element) => {
    const visible = window.siyuan.config.sync.enabled;
...
export const getLANSyncSearchAvailability = () => {
    if (window.siyuan.config.system.container === "docker") {
...
const setSyncModeRelatedConfigItemVisible = (root: Element) => {
    ...
    const isAutomaticSync = syncMode === 1;
    root.querySelector(`#${CSS.escape("sync.interval")}`)?.closest(".config-item")?.classList.toggle("fn__none", !isAutomaticSync);
    root.querySelector(`#${CSS.escape("sync.perception")}`)?.closest(".config-item")?.classList.toggle("fn__none", !(isAutomaticSync && window.siyuan.config.system.container !== "docker"));
};
